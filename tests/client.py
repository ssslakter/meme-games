...
class TestWebClient:
    ...
    def __init__(self, url="http://testserver", timeout=60):
        self.base_url = url
    ...
    def _sync(self, method, url, **kwargs):
        async def _request():
            return await self.cli.request(method, url, **dict(timeout=self.timeout, **kwargs))
    ...
    def connect_ws(self, ws_url):
        ...
        async def _connect(): 
            self.ws = await wait_for(websockets.connect(ws_url, additional_headers=headers), self.timeout)

...
async def send(u: BaseLobbyMember, fn, *args, json=False):
...
async def notify_all(lobby: Lobby, fn: Callable[[BaseLobbyMember, Lobby, tuple[Any, ...]], Any], *args, filter_fn=None, json=False):
    ...
    tasks = [send(u, fn, lobby, *args, json=json) for u in lobby.sorted_members() if u.is_connected and (filter_fn is None or filter_fn(u))]

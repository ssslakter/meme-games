...
# %% auto 0
__all__ = ['logger', 'LOBBY_REGISTRY', 'MEMBER_MANAGER_REGISTRY', 'GameData', 'LobbyMember', 'MemberManager', 'register_manager',
           'Lobby', 'LobbyManager', 'LobbyService', 'is_player', 'lobby_beforeware', 'Members']
...
def register_manager(manager: DataManager, entity_cls: type):
...
class Lobby[T: LobbyMember](Model):
    ...
    def _asdict(self):
        res =  super()._asdict()
        res['lobby_type'] = dict_inverse(LOBBY_REGISTRY)[res.pop('member_type')]
        return res
    
    @classmethod
    def columns(cls):
        res =  super().columns()
        res['lobby_type'] = str; res.pop('member_type')
        return res
    ...
    @classmethod
    def from_cols(cls, data: dict):
        data['member_type'] = LOBBY_REGISTRY[data.pop('lobby_type')]
        return super().from_cols(data)
...
class LobbyManager(DataManager[Lobby]):
    ...
    def update(self, lobby: Lobby[LobbyMember]):
        self.memm[lobby.member_type].upsert_all(lobby.members.values())
    ...
    def get(self, id: str) -> Lobby[LobbyMember]:
        ...
        lobby.members = {m.user_uid: m for m in self.memm[lobby.member_type].get_all(id)}

...
def NotesBlock(reciever: LobbyMember | User, lobby: Lobby, **kwargs):
    if not isinstance(reciever, LobbyMember) or not reciever.is_player:
        return Div(id='notes-block', **kwargs)
    note = NotesCard(reciever, reciever, lobby.state.player(reciever.uid), lobby.state)
    return Div(note, id='notes-block', cls='draggable-panel mg-floating-notes', **kwargs)

...
def ws_fn(connected=True, render_fn: Callable = JoinSpectators):
    ...
    async def user_joined(sess, send, ws):
        ...
        if not lobby.game_state: lobby.game_state = AliasGameState()
        if not lobby: return
...
async def new_team(req: Request):
    ...
    game_state: AliasGameState = lobby.game_state
...
def index(req: Request, lobby_id: str = None):
    ...
    lobby, was_created = lobby_service.get_or_create(u, lobby_id, AliasPlayer)
    if was_created: 
        lobby_service.update(lobby)
    if not lobby.game_state: lobby.game_state = AliasGameState()

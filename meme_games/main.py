...
DI.register_services([UserManager, LobbyManager, MemberManager, WhoAmIManager, CodenamesManager, LobbyService])

register_lobby_member_manager(DI.get(MemberManager), LobbyMember)
register_lobby_member_manager(DI.get(WhoAmIManager), WhoAmIPlayer)
register_lobby_member_manager(DI.get(CodenamesManager), CodenamesPlayer)
...
for rt in [shared_rt, whoami_rt, video_rt, word_packs_rt, codenames_rt]:
    rt.to_app(app)
